...
#             param_frames[0].pack(anchor="w", pady=2)
...
#         entry = Entry(frame, width=entry_width)
#         entry.pack(side="left", padx=10)
#         frame.pack_forget()  # ukryj domyślnie
...
#     btn_convert.pack(side="top", pady=15)
#
#     # Pierwsze ustawienie widoczności
#     update_visibility()
#
#     # Wykres
#     frame_plot = Frame(frame_main)
...
#         new_window.signal_data = (time_l, signal_l, signal_type_l)
#         plot_signal(ax, time_l, signal_l, signal_type_l, f"Wczytany sygnał {signal_type_l}")
#         new_window.original_data = (time_l, signal_l)
#         canvas.draw()
#
#     btn_save = Button(frame_buttons, text="Zapisz sygnał", command=save_signal_window)
#     btn_save.pack(side="left", padx=(0, 5))
...
#
#         # Ustaw parametry i pokaż odpowiednie pola
#         if selected == "Próbkowanie":
...
#
#     # Jeden label na miary błędu, początkowo pusty
#     error_metrics_label = Label(frame_bottom, text="", font=("Arial", 10))
#     error_metrics_label.pack(side="top", padx=10)
#     new_window.error_metrics_label = error_metrics_label
#
...
#         plot_signal(ax, *new_data, title=label)
#
#         # Obliczenie miar błędów – tylko dla rekonstrukcji
#         if conversion_type.get() == "Rekonstrukcja":
#             # Oryginalny sygnał przed próbkowaniem (pełny)
#             if hasattr(new_window, "original_data"):
#                 original_time, original_signal = new_window.original_data
#                 # Dopasuj długości – zakładamy, że t_full pokrywa się z original_time
...
#         else:
#             error_metrics_label.config(text="")  # Czyść jeśli nie rekonstrukcja
#
#         # Po próbkowaniu
...
#
#     # === Tworzenie pól na parametry konwersji ===
#     param_frames = {}
#     param_labels = {}
#
#     for i, param in enumerate(param_names):
#         frame = Frame(frame_params)
#         label_var = StringVar(value=param)
#         Label(frame, textvariable=label_var, width=label_width, anchor="w").pack(side="top", padx=(13, 0))
#         entry = Entry(frame, width=entry_width)
#         entry.pack(side="left", padx=10)
#         frame.pack_forget()
#         param_name = f"param{i + 1}"
#         param_frames[param_name] = frame
#         param_labels[i] = label_var
#         conversion_param_entries[param] = entry
#
#     # === Przycisk konwertuj ===
#     btn_convert = Button(frame_params, text="Konwertuj", command=on_convert)
